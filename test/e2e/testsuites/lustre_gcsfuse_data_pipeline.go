...
func (t *gcsFuseCSILustreDataPipelineTestSuite) DefineTests(driver storageframework.TestDriver, pattern storageframework.TestPattern) {
	...
	ginkgo.It("should stage data from the GCS Fuse mount into a Lustre-backed volume with content integrity", func() {
	...
	// fsync/close.
	ginkgo.It("should export a checkpoint file from Lustre to the GCS Fuse mount with content integrity and bucket visibility", func() {
	...
	ginkgo.It("should handle concurrent multi-writer pods writing to the same Lustre volume and GCS Fuse bucket without corruption", func() {
	...
	ginkgo.It("should coexist with mixed Lustre-only, GCSFuse-only, and Lustre+GCSFuse pods on the same node without resource conflicts", func() {
